...
std::vector<std::vector<float>> mul(std::vector<std::vector<float>> M1, std::vector<std::vector<float>> M2) {
	std::vector<std::vector<float>> MR(sz(M1), std::vector<float>(sz(M1)));
	for (int i = 0; i < sz(M1); ++i) {
		for (int j = 0; j < sz(M1[i]); ++j) {
			float r = 0.0;
			for (int f = 0; f < sz(M1[j]); ++f) {
				r += (M1[i][f] * M2[f][j]);
			}
			MR[i][j] = r;
		}
...
public:
	...
	void Template(Poligon& P, std::vector<std::vector<float>> M) {
		for (int i = 0; i < sz(P.Points); ++i) {
			std::cout << "  Vertice como Vector " << i << " Tranpuesto resultante" << std::endl;
			std::cout << "    ";
			PrintPTO(P.Points[i]);
			std::cout << "      |" << std::endl;
			std::cout << "      V" << std::endl;
			P.Points[i] = mul(P.Points[i], M);
			std::cout << "    ";
			PrintPTO(P.Points[i]);
		}
	}
